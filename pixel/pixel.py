...
class ImageUtil:
        ...
        for i in range(0, rows, step):
            for j in range(0, cols, step):
                ...
                if h1 - h2 >= 15:
                    ...
                    img1[i : i + step][j : j + step] = (
                        sec1 * 0.25 + sec2 * 0.25 + COLOR_PINK * 0.5
                    )
        endtime = time.time()
            ...
            for j in range(cols):
                ...
                if not ImageUtil.__is_similar_color(img1[i][j], scaledImg[i][j]):
                    img1[i][j] = np.array(
                        [
                            img1[i][j][0] * 0.25
                            + scaledImg[i][j][0] * 0.25
                            + COLOR_PINK[0] * 0.5,
                            img1[i][j][1] * 0.25
                            + scaledImg[i][j][1] * 0.25
                            + COLOR_PINK[1] * 0.5,
                            img1[i][j][2] * 0.25
                            + scaledImg[i][j][2] * 0.25
                            + COLOR_PINK[2] * 0.5,
                        ],
                        np.uint8,
                    )
    ...
    def __is_similar_color(cls, color1, color2):
        ...
        b2, r2, g2 = color2
        if (
            abs(int(b1) - int(b2)) < 15
            and abs(int(r1) - int(r2)) < 15
            and abs(int(g1) - int(g2)) < 15
        ):
            return True
        else:
            return False
